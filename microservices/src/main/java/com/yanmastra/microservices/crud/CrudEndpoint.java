...
import com.yanmastra.microservices.configuration.HttpException;
...
public abstract class CrudEndpoint<Entity extends CrudableEntity, Dao> {
    ...
    private static final Logger logger = LogManager.getLogger(CrudEndpoint.class);


    @GetMapping
    @Transactional
    public List<Dao> getList(
            @RequestParam(value = "page", defaultValue = "1", required = false) Integer page,
            @RequestParam(value = "size", defaultValue = "5", required = false) Integer size,
            Principal principal
    ) {
        Pageable pageable = Pageable.ofSize(size).withPage(page);
        return getRepository().findAll(pageable).map(this::fromEntity).toList();
    }
